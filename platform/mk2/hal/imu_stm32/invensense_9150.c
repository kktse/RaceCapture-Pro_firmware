...
static int is9150_init_magnetometer(void)
{
    int res = bypass_on();
    if (res) {
...
int is9150_init(struct i2c_dev *dev, uint8_t addr, uint8_t mag_addr)
{
    int res;
    uint8_t reg = 0x00;
    ...
    res = is9150_init_magnetometer();
    if (res) {
            pr_error("IMU: failed magnetometer setup\r\n");
            return IS_9150_ERR_INIT;
    }
